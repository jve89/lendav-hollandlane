...
- Keep client-side JavaScript at zero unless a phase explicitly calls for it. **There are exactly three named exceptions, one of which is planned rather than built, and the list is closed the same way the dependency list is.** Anything not on it needs approval; anything on it must not be deleted as a violation:
  ...
  3. **The hero video's viewport-entry playback — PLANNED, NOT BUILT.** Approved 27 July 2026; the constraints, the budget and the acceptance gate are in PLAN's Phase 10, and nothing is written against it until there is real footage. **It passes the test item 2 sets**: with JavaScript disabled nothing triggers playback, the `preload="none"` video is never fetched, and the visitor sees the poster still — which is a finished state by design, not a broken one. It defers *when the video plays*, never *what the page loads*. Reasoning in ARCHITECTURE section 2; the Hero contract in section 6 still describes the no-script form and is reconciled by the session that builds it.
