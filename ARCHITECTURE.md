...
**Version 2.1 · 8 August 2026** — **`PathChooser` is gone, one day old, and what
replaced it is three pointers instead of one fork.** Inspection is now reached
from a full section on both home pages (after the before/after evidence, before
`CompareTable`), a note under the services grid, and a block under the price
table — `InspectionSection`, `InspectionNote` and `InspectionPricing`, three
components rather than one with a size prop, because they differ in what they
have to say. Section 6 carries all three contracts and loses `PathChooser`.
**`inspection` is no longer in `navOrder`**, which is also what keeps it out of
the footer, since `Footer.astro` renders `liveNav` too — recorded in section 3
with the reasoning, because an absent nav entry looks like an oversight.
`PriceTable` gains its first visible heading. **SPEC section 11 now carries the
governing rule — POINTER, NEVER MEMBER — and this document does not restate it.**

**Version 2.0 · 8 August 2026** — **a second line of business was added and
...
pairing code at all. Section 4 lists `i18n/inspection.ts` and the two new pages.
Section 6 records
...
is untouched. **SPEC section 11 is new and
governs what the inspection page may say**; it is stricter than anything else on
this site and this document does not restate it. *(The `PathChooser` contract
this version added was removed again by 2.1 above, one day later.)*
...
**The page is NOT in the navigation, and that is a decision — `navOrder` and
`builtRoutes` are two different lists doing two different jobs.** `inspection` is
in `builtRoutes`, so `path()` resolves it, the page is built, `npm run links`
guards it and it is indexed and hreflang-paired like any other. It is **not** in
`navOrder`, so the menu is five items. It is reached instead from three in-page
pointers — a section on both home pages, a note under the services grid, a block
under the price table — each arriving beside the washing content it relates to,
where it has already earned its context; a sixth nav entry would compete with the
five carrying the washing journey and give a visitor no reason to click it.

**One mechanism, not two lists: `Footer.astro` renders `liveNav` as well**, so
leaving the key out of `navOrder` is what keeps inspection out of the footer
too. There is nothing else to remember, and nothing to remove separately. This is
written here rather than left to be inferred because **an absent nav entry looks
exactly like an oversight**, and the obvious "fix" is one line. SPEC section 11
carries the rule it follows from.
...
└── src/
    ...
    │   ├── InspectionSection.astro  # pointer 1 — home pages, after the evidence section
    │   ├── InspectionNote.astro     # pointer 2 — under the services grid. NOT a card
    │   ├── InspectionPricing.astro  # pointer 3 — under the price table. NOT a row
    │   ├── QuoteForm.astro
    │   └── Cta.astro
    ├── content/
    │   ├── services/
...
- **`InspectionSection` / `InspectionNote` / `InspectionPricing`** — each props `{ locale }`. **The three pointers to the inspection line**, added 8 August 2026 and replacing `PathChooser` (below). Each takes only `locale`, like `TrustBar` and `Cta`; each reads its copy from `inspectionPointer(surface, locale)` in `i18n/inspection.ts` and resolves its one link through `path('inspection', locale)`, so no URL and no prose is typed in a component. **All three are governed by SPEC section 11's POINTER-NEVER-MEMBER rule**, and each expresses it structurally rather than by comment:
  - **`InspectionSection`** — home pages, **after the before/after evidence and before `CompareTable`**. That position is load-bearing: services → evidence is the washing argument start to finish and nothing goes between them, so this arrives once that argument has landed. A `.card` on an otherwise flat page, which is what says "different line of business" without a band, a rule or a colour.
  - **`InspectionNote`** — under the six-card grid on `/teenused`. **Not a card**, deliberately: anything card-shaped directly beneath that grid reads as a seventh washing service. A plain block with a rule above it, visibly outside the grid.
  - **`InspectionPricing`** — under the washing price table on `/hinnakiri`. **Not a row**, and this is the sharpest form of the rule: every row of that table carries a per-m² from-price or a `priceKind` resolved against `site.prices`, so an inspection row would imply a square-metre price for work that is quoted per job — and `PriceTable` could not render one anyway without inspection becoming a `serviceKey`. **No figure appears in it and none may be added.**

  **Three components, not one with a size prop.** They differ in length, in shape and in what they must answer — only the pricing block has to say what it costs — so unifying them would mean a prop deciding how much copy to render. **Zero client JavaScript**, no new colour values: `.card`, `--line` and the existing type scale.
- **`PathChooser` — BUILT AND DELETED ON 8 AUGUST 2026, and it is not coming back.** Two cards under the hero, cleaning or inspection, asking the visitor to pick. It was the wrong shape: **it forced an either/or between two things that are frequently sequential** — somebody unsure whether a wash is needed can have the property looked at first — and it spent the top of the page on the smaller line of business. The three pointers above replaced it. The reasoning is in SPEC section 11 and in a tombstone comment in `i18n/home.ts` where its copy lived; this entry exists so the component is not reinvented from the shape of the problem.
