---
title: Gutters and downpipes
slug: gutter-cleaning
locale: en
summary: Leaves, needles and moss cleared from gutters and downpipes. Booked together with a roof clean, on the same visit.
priceKind: addon
priceNote: Gutter clearing is booked as an add-on to a roof clean. The price depends on the length of the guttering and the height of the roof, and we give it in the roof cleaning quote.
order: 5
icon: gutter
seoTitle: Gutter and downpipe clearing
seoDescription: Gutters and downpipes cleared as an add-on to a roof clean. Across Estonia, without anyone stepping on the roof.
faqRefs: []
---

<!-- needs-operator-review -->

## What gutter clearing covers

We clear leaves, needles, moss and silt out of the gutters and downpipes, and
check that the water runs through.

A blocked gutter sends water over the edge, against the wall and down beside the
foundation. Preventing that costs less than repairing it later.

## How the work is done

Gutter clearing is booked as an add-on to a roof clean, on the same visit. We do
not currently offer it as a visit of its own, because one short job does not
cover the cost of the trip.

<!-- unconfirmed: the method used to clear the gutters is not settled. Do not
     describe a method, a tool or an attachment here until it is. PLAN Phase 10. -->

We describe how the work is done, and with what, in the roof cleaning quote.

## What gutter clearing does not cover

We do not repair guttering. Loose brackets, gutters running to the wrong fall,
leaking joints and broken pipes need a sheet metal or roofing company. If we see
something like that while working, we tell you, but we do not fix it ourselves.

The service also does not include fitting gutter guards, clearing drainage and
surface water gullies, or removing ice and snow in winter.

## Cleaning products

<!-- unconfirmed: the cleaning product and its Estonian biocide authorisation
     are not confirmed. Do not name a product here, and do not make any claim
     about one, until they are. PLAN Phase 10 replaces this section. -->

We name the product we use on your property, and the Estonian biocide
authorisation it is approved under, in your quote.

## How long the work takes

<!-- unconfirmed: no job durations have been measured — the first jobs are not
     done. Do not publish a figure here until there is one. PLAN Phase 10. -->

We give the duration in your quote, for your property specifically. We publish
no general figure: it depends on the property, and we have no measurements of
our own to base one on yet.
